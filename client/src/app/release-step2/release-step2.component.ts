...
export class ReleaseStep2Component {
  ...
  ngOnInit(): void {
    ...
    if (this.state.wiToBranch) {
      for (const wi in this.state.wiToBranch) {
        if(this.state.computedBranches.includes(this.state.wiToBranch[wi][0])){
    ...
    this.checkAmb = this.ambiguousSelections && Object.keys(this.ambiguousSelections).length > 0;
  ...
  computeFinalBranches(): void {
    ...
    if(finalSet.size == 0) {
      finalSet = new Set(this.state.computedBranches);
    }
